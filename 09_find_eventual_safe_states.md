...
- ~~Once I identified the parent and terminal node I'll append them in the resulatant array, I'll do this for all the veritices~~
- ~~And finally I'll return the sorted resulatant array.~~
- There was a logical flaw in my initial thoughts 
    - Imagine a node $A$ that acts as a fork Ψ
        - Path 1 goes to the terminal node
        - Path 2 goes into an endless cycle
    - If we only look to see if node $A$ connect to a terminal node, we might mistakenly call it safe, but actual $A$ is unsafe
- Instead of looking for terminal node, we can look for cycles, So this problem is basically disguised version of __Cycle Detection in a Directed Graph__
